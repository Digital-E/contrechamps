...
const homeFields = `
  ...
  "videoMp4": videoMp4.asset->url,
  "image": image,
  "videoMp4Mobile": videoMp4Mobile.asset->url,
...
const inscriptionNewsletterFields = `
  _id,
  title,
  content,
  text,
...
export const indexQueryPreview = `
*[_type == "post" && _lang == $slug] | order(startdate asc) {
  ${postFields}
}
`
...
export const actualitesQueryPreview = `
*[_type == "news" && _lang == $slug] | order(startdate asc) {
  ${actualiteFields}
}
`
...
*[_type == "photo" && _lang == $lang && !(_id in path('drafts.**'))] | order(date desc) {
    ...
    image,
    text,
    "hasGallery": count(slices[_type == "image"]) > 0,
    "slug": slug.current
}
`
...
export const orderedMediaListQuery = `
*[_type == "mediaPage" && slug.current == $slug && !(_id in path('drafts.**'))] [0] {
  mediaItems[]->
}`
...
export const inclusiviteSlugsQuery = `
*[_type == "inclusivite" && defined(slug.current)][].slug.current
`

export const previewInclusiviteQuery = `
*[_type == "inclusivite" && slug.current == $slug] | order(_updatedAt desc) [0] {
    ${lEnsembleFields}
}`

export const inclusiviteQuery = `
*[_type == "inclusivite" && slug.current == $slug && !(_id in path('drafts.**'))] | order(_updatedAt desc) [0] {
    ${lEnsembleFields}
}`

export const inclusiviteMenuQuery = `
*[_type == "inclusiviteMenu" && _lang == $lang][0] {
  menuItems
}`

export const previewInscriptionNewsletterQuery = `
*[_type == "inscriptionNewsletter" && slug.current == $slug] | order(_updatedAt desc) [0] {
    ${inscriptionNewsletterFields}
}
`

export const inscriptionNewsletterQuery = `
*[_type == "inscriptionNewsletter" && slug.current == $slug && !(_id in path('drafts.**'))] | order(_updatedAt desc) [0] {
    ${inscriptionNewsletterFields}
}
`